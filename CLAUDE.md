...
| `emkd` / `em_kd` | `EMKDCriterion` | Reverse-KL on response logits + Hungarian-matched vision-logit distillation + vision↔text affinity. Reads `em_kd_alpha/beta/gamma/temperature` from CLI args. Optional `em_kd_max_vision_tokens` / `em_kd_max_text_tokens` budget caps to keep Hungarian tractable. |
| `sre` | `SRECriterion` | Weighted cosine span loss + geometry MSE on similarity matrices + shared-vocab soft-label KL. Hidden alignment via learnable student→teacher projector (`--sre_use_projector true`, looked up from `distiller.projectors[idx]`) — falls back to `_crop_last_dim` only when no projector is provisioned. |
| `joint` / `unit_aligned` / `unit_aligned_distillation` | `UnitAlignedDistillationCriterion` | Joint SRE + EM-KD, single shared student/teacher forward pass per step. Weighted by `joint_ce_weight`, `joint_emkd_weight`, `joint_sre_weight`. |
...
`EMKDCriterion` and `SRECriterion` expose `compute_losses(distiller, student_outputs, teacher_outputs, student_inputs, teacher_inputs)` alongside `forward(distiller, batch)` — the former lets the joint criterion call them without re-running the student/teacher forward.

Loss weights live on `TrainingArguments` (`src/arguments.py`): `kd_weight` (default criterion); `em_kd_alpha/beta/gamma/temperature` and optional `em_kd_max_vision_tokens` / `em_kd_max_text_tokens`; `sre_alpha/p/span_loss_weight/geom_loss_weight/logit_loss_weight/temperature/use_projector`; `joint_ce_weight/emkd_weight/sre_weight`.
